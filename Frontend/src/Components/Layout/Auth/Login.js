...
const { decodedToken } = useJwt(token);
 ...
 //S5.7-- Create signIn & Register Function with EVENT, NOW GO TO FIREBASE.COM, ENABLE signIn Method Email/Password and INSTALL FIREBASE NOW(BASH), go to firebase.js
  const signIn = e => {
    e.preventDefault(); //the preventDefault Event will PREVENT from REFRESHING of Page

   //S6.3--FIREBASE SignIn STUFF
    auth
        .signInWithEmailAndPassword(email, password)
        .then((auth) => {
          if(auth.user.email) {
            // console.log("Already Inside the LocalStorage")
